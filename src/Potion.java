...
public class Potion extends GameObj {
    ...
    public Potion(int courtWidth, int courtHeight, int px) {
    	super(px, py, SIZE, SIZE, courtWidth, courtHeight);
        
        try {
            img = ImageIO.read(new File("files/magicpotion.png"));
        }
        catch(IOException e) {
            e.printStackTrace();
        }
    ...
    public void changeState(Cat c) {
    	c.switchInvincible();
		c.setCurrScore(c.getCurrScore()+10);
    }
}